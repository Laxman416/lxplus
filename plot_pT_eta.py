...
ax.errorbar(x_value, asymmetry, yerr=asymmetry_error, fmt='o', capsize=5, color = 'blue')
if args.scheme == 'pT':
    ax.set_xlabel('pT [GeV]', fontsize = 16)
elif args.scheme == 'eta':
    ax.set_xlabel('eta', fontsize = 16)
ax.set_ylabel('Asymmetry [%]', fontsize = 16)
ax.tick_params(axis='both', which='both', labelsize=12)

ax.axhline(binned_asymm, color='black', linestyle='dashed', linewidth=1.5, label = r'Integrated $A_{\mathrm{prod}}$')
plt.axhspan(binned_asymm-binned_asymm_error, binned_asymm+binned_asymm_error, color='red', alpha=0.4, lw=0, label = r'Error on Intergrated $A_{\mathrm{prod}}$')

ax.axhline(unbinned_asymm, color='red', linestyle='dashed', linewidth=1.5, label = r'$A_{\mathrm{prod}}$')
plt.axhspan(unbinned_asymm-unbinned_asymm_error, unbinned_asymm+unbinned_asymm_error, color='yellow', alpha=0.3, lw=0, label = r'Error on $A_{\mathrm{prod}}$')
